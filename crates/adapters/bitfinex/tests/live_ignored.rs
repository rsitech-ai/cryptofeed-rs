...
use marketfeed_adapter_bitfinex::{BITFINEX_DERIV_VENUE_ID, BitfinexDerivFactory, BitfinexFactory};
...
#[tokio::test]
#[ignore = "live network: cargo test -p marketfeed-adapter-bitfinex --test live_ignored -- --ignored --nocapture"]
async fn live_bitfinex_spot_trade_or_quote() {
    let factory = BitfinexFactory { enable_l2: false };
    let plan = factory
        .plan(
            &Default::default(),
...
#[tokio::test]
#[ignore = "live network"]
async fn live_bitfinex_deriv_trade_or_mark() {
    let factory = BitfinexDerivFactory { enable_l2: false };
    let plan = factory
        .plan(
            &Default::default(),
    ...
    supervisor
        ...
        .unwrap();
    let mut ws = TungsteniteWebSocket::new();
    let http = ReqwestHttpTransport::new().expect("reqwest");
    let spec = WebSocketSpec {
        url,
        max_frame_bytes: 4 * 1024 * 1024,
        ..WebSocketSpec::default()
    ...
    let run = supervisor.run_session_loop(session, &mut ws, &http, &spec, policy, 2);
    tokio::select! { res = run => { res.unwrap(); } _ = tokio::time::sleep(std::time::Duration::from_secs(20)) => {} }
    let runner = supervisor.session_mut(session).unwrap();
    assert!(
        runner
            .market_batches
            .iter()
            .any(|b| b.events.iter().any(|e| matches!(
                e.payload,
                MarketEvent::Trade(_)
                    | MarketEvent::Quote(_)
                    | MarketEvent::MarkPrice(_)
                    | MarketEvent::Funding(_)
            )))
    );
}
