//! Checked-in raw replay corpus for Bitstamp spot L2 + REST candles (offline CI).
//!
//! Regenerate with:
//! `REGEN_CORPUS=1 cargo test -p marketfeed-adapter-bitstamp --test corpus_replay regen_spot_l2_book_corpus -- --ignored`
//! `REGEN_CORPUS=1 cargo test -p marketfeed-adapter-bitstamp --test corpus_replay regen_spot_candles_rest_corpus -- --ignored`

use bytes::Bytes;
use marketfeed_adapter_api::{CandleInterval, ConcreteSubscriptionSet, HttpResponse, SessionSpec};
use marketfeed_adapter_bitstamp::{BitstampSession, BitstampSessionConfig};
...
/// order_book snapshot + two diff_order_book deltas (upsert then delete).
fn l2_corpus_frames() -> [&'static str; 3] {
    [
        r#"{"channel":"order_book_btcusd","event":"data","data":{"timestamp":"1609459200","microtimestamp":"1609459200123456","bids":[["29000.00","1.50000000"]],"asks":[["29001.00","2.00000000"]]}}"#,
        r#"{"channel":"diff_order_book_btcusd","event":"data","data":{"timestamp":"1609459201","microtimestamp":"1609459201123456","bids":[["29000.00","1.80000000"]],"asks":[]}}"#,
        r#"{"channel":"diff_order_book_btcusd","event":"data","data":{"timestamp":"1609459202","microtimestamp":"1609459202123456","bids":[],"asks":[["29001.00","0"]]}}"#,
    ]
}
...
#[test]
#[ignore = "regen only: REGEN_CORPUS=1 cargo test -p marketfeed-adapter-bitstamp --test corpus_replay regen_spot_l2_book_corpus -- --ignored"]
...
fn candle_rest_body() -> &'static [u8] {
    br#"{"data":{"pair":"BTC/USD","ohlc":[{"timestamp":"1609459200","open":"0.0010","high":"0.0025","low":"0.0015","close":"0.0020","volume":"1000"}]}}"#
}
...
fn candle_session() -> BitstampSession {
    let mut ids = HashMap::new();
    ids.insert("btcusd".into(), InstrumentId(1));
    BitstampSession::new(
        SessionSpec {
...
fn drive_candle_http(body: &[u8]) -> Vec<MarketEvent> {
    ...
    let req_id = live
        .take_pending_http()
        .into_iter()
        .find(|r| r.url.contains("/ohlc/"))
        .map(|r| r.id)
        .expect("bitstamp ohlc request");
...
#[test]
#[ignore = "regen only: REGEN_CORPUS=1 cargo test -p marketfeed-adapter-bitstamp --test corpus_replay regen_spot_candles_rest_corpus -- --ignored"]
