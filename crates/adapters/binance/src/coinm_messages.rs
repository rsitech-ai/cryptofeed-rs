//! Binance Coin-M JSON decode (aggTrade / bookTicker / ticker / mark / indexPrice / depth / forceOrder / OI / kline).
...
pub enum CoinmDecoded {
    ...
    /// `@ticker` / `24hrTicker` — rolling 24h stats.
    ///
    /// COIN-M's production payload does not include BBO fields. Quotes arrive
    /// independently on `@bookTicker`.
    Ticker24h {
        symbol: String,
        open: Price,
        high: Price,
    ...
    /// Dedicated `<pair>@indexPrice@1s` (pair = base+quote, e.g. `BTCUSD` for `BTCUSD_PERP`).
    IndexPrice {
        /// Pair id from wire `i` (e.g. `BTCUSD`), not the contract symbol.
        pair: String,
        price: Price,
        exchange_ts_ms: i64,
    },
    ...
    DepthUpdate {
        ...
        first_update_id: u64,
        final_update_id: u64,
        /// Previous event's `u` — futures continuity field (same as USD-M).
    ...
    /// REST `/dapi/v1/openInterest` body (Coin-M has no public OI WS stream).
    OpenInterest {
        symbol: String,
        quantity: Quantity,
        exchange_ts_ms: i64,
...
/// Decode Coin-M WS/REST JSON into a canonical [`CoinmDecoded`].
///
...
pub fn decode_text(bytes: &[u8]) -> Result<CoinmDecoded, String> {
    let v: Value = crate::json::value_from_slice(bytes)?;
    decode_value(&v)
}

/// Reference decode that always uses `serde_json` (parity oracle).
pub fn decode_text_serde(bytes: &[u8]) -> Result<CoinmDecoded, String> {
...
pub fn decode_text_simd(bytes: &[u8]) -> Result<CoinmDecoded, String> {
    let v: Value = crate::json::value_from_slice_simd(bytes)?;
    decode_value(&v)
}

fn decode_value(v: &Value) -> Result<CoinmDecoded, String> {
    if let Some(obj) = v.as_object() {
        ...
        if obj.contains_key("result") && (obj.contains_key("id") || obj.len() <= 2) {
            ...
            return Ok(CoinmDecoded::SubscribeAck { id: ack.id });
        }
    ...
    Ok(CoinmDecoded::Unknown)
}

fn decode_kline(v: &Value) -> Result<CoinmDecoded, String> {
    let m: KlineMsg = serde_json::from_value(v.clone()).map_err(|e| e.to_string())?;
    Ok(CoinmDecoded::Candle {
        symbol: m.s,
...
fn decode_agg_trade(v: &Value) -> Result<CoinmDecoded, String> {
...
fn decode_book_ticker(v: &Value) -> Result<CoinmDecoded, String> {
    let m: BookTickerMsg = serde_json::from_value(v.clone()).map_err(|e| e.to_string())?;
    Ok(CoinmDecoded::Quote {
        symbol: m.s,
        update_id: m.u,
...
fn decode_24hr_ticker(v: &Value) -> Result<CoinmDecoded, String> {
    let m: Ticker24hMsg = serde_json::from_value(v.clone()).map_err(|e| e.to_string())?;
    Ok(CoinmDecoded::Ticker24h {
        symbol: m.s,
...
fn decode_mark_price(v: &Value) -> Result<CoinmDecoded, String> {
    let m: MarkPriceMsg = serde_json::from_value(v.clone()).map_err(|e| e.to_string())?;
    Ok(CoinmDecoded::MarkPrice {
        symbol: m.s,
...
struct IndexPriceMsg {
    #[serde(rename = "E")]
    event_time: i64,
    /// Pair (Coin-M: `BTCUSD`), not contract symbol.
    i: String,
    p: String,
}

fn decode_index_price(v: &Value) -> Result<CoinmDecoded, String> {
    let m: IndexPriceMsg = serde_json::from_value(v.clone()).map_err(|e| e.to_string())?;
    Ok(CoinmDecoded::IndexPrice {
        pair: m.i,
        price: Price(parse_fixed(&m.p)?),
        exchange_ts_ms: m.event_time,
    })
...
/// Coin-M index stream uses pair id (`BTCUSD`), not contract (`BTCUSD_PERP`).
pub fn coinm_index_pair(symbol: &str) -> &str {
    symbol.split('_').next().unwrap_or(symbol)
}

fn decode_depth_update(v: &Value) -> Result<CoinmDecoded, String> {
    let m: DepthUpdateMsg = serde_json::from_value(v.clone()).map_err(|e| e.to_string())?;
    Ok(CoinmDecoded::DepthUpdate {
        symbol: m.s,
...
fn decode_snapshot(v: &Value) -> Result<CoinmDecoded, String> {
...
fn decode_open_interest(v: &Value) -> Result<CoinmDecoded, String> {
    let m: OpenInterestMsg = serde_json::from_value(v.clone()).map_err(|e| e.to_string())?;
    Ok(CoinmDecoded::OpenInterest {
        symbol: m.symbol,
...
fn decode_force_order(v: &Value) -> Result<CoinmDecoded, String> {
...
mod tests {
    ...
    fn decode_kline_exact_fixed() {
        let raw = br#"{"e":"kline","E":123456789,"s":"BTCUSD_PERP","k":{"t":123400000,"T":123460000,"s":"BTCUSD_PERP","i":"1m","f":100,"L":200,"o":"0.0010","c":"0.0020","h":"0.0025","l":"0.0015","v":"1000","n":100,"x":true,"q":"1.0000","V":"500","Q":"0.500","B":"123456"}}"#;
        let CoinmDecoded::Candle {
            open,
            close,
            interval_ns,
            start_ts,
    ...
    fn decode_agg_trade_and_mark() {
        let raw = br#"{"e":"aggTrade","E":1,"s":"BTCUSD_PERP","a":9,"p":"65000.1","q":"1","f":1,"l":2,"T":3,"m":true}"#;
        let CoinmDecoded::AggTrade {
            aggressor, price, ..
        } = decode_text(raw).unwrap()
        else {
            panic!("aggTrade");
        ...
        let mark = br#"{"e":"markPriceUpdate","E":10,"s":"BTCUSD_PERP","p":"65000.00","i":"64990.00","P":"65001.00","r":"0.00010000","T":20}"#;
        let CoinmDecoded::MarkPrice {
            funding_rate,
            next_funding_ts,
            ..
        ...
        let idx = br#"{"e":"indexPriceUpdate","E":11,"i":"BTCUSD","p":"64991.00"}"#;
        let CoinmDecoded::IndexPrice { pair, price, .. } = decode_text(idx).unwrap() else {
            panic!("indexPriceUpdate");
        };
        assert_eq!(pair, "BTCUSD");
        assert_eq!(price.0, Fixed::new(6499100, 2));
        assert_eq!(coinm_index_pair("BTCUSD_PERP"), "BTCUSD");
        assert_eq!(coinm_index_pair("BTCUSD_250926"), "BTCUSD");
    }

    #[test]
    fn decode_depth_pu_and_snapshot() {
        let depth = br#"{"e":"depthUpdate","E":1,"T":2,"s":"BTCUSD_PERP","U":10,"u":12,"pu":9,"b":[["100.0","0"]],"a":[["101.0","1"]]}"#;
        let CoinmDecoded::DepthUpdate {
        ...
        let snap = br#"{"lastUpdateId":100,"bids":[["100.0","1"]],"asks":[["101.0","2"]]}"#;
        let CoinmDecoded::DepthSnapshot {
            last_update_id,
            bids,
            ..
    ...
    fn decode_text_matches_serde_oracle() {
        for raw in coinm_parity_fixtures() {
            assert_eq!(
                decode_text(raw).unwrap(),
    ...
    fn coinm_parity_fixtures() -> &'static [&'static [u8]] {
        &[
            br#"{"e":"aggTrade","E":1,"s":"BTCUSD_PERP","a":9,"p":"65000.1","q":"1","f":1,"l":2,"T":3,"m":true}"#,
            br#"{"stream":"btcusd_perp@aggTrade","data":{"e":"aggTrade","E":1,"s":"BTCUSD_PERP","a":10,"p":"65000.0","q":"2","f":3,"l":4,"T":4,"m":false}}"#,
            br#"{"u":9,"s":"BTCUSD_PERP","b":"65000.0","B":"1","a":"65000.1","A":"2"}"#,
            br#"{"e":"markPriceUpdate","E":10,"s":"BTCUSD_PERP","p":"65000.00","i":"64990.00","P":"65001.00","r":"0.00010000","T":20}"#,
            br#"{"e":"indexPriceUpdate","E":11,"i":"BTCUSD","p":"64991.00"}"#,
            br#"{"e":"depthUpdate","E":1,"T":2,"s":"BTCUSD_PERP","U":10,"u":12,"pu":9,"b":[["100.0","0"]],"a":[["101.0","1"]]}"#,
            ...
            br#"{"e":"kline","E":123456789,"s":"BTCUSD_PERP","k":{"t":123400000,"T":123460000,"s":"BTCUSD_PERP","i":"1m","f":100,"L":200,"o":"0.0010","c":"0.0020","h":"0.0025","l":"0.0015","v":"1000","n":100,"x":true,"q":"1.0000","V":"500","Q":"0.500","B":"123456"}}"#,
            br#"{"result":null,"id":1}"#,
        ]
    }

    #[test]
    fn decode_quote_force_order_and_oi() {
        let quote = br#"{"u":9,"s":"BTCUSD_PERP","b":"65000.0","B":"1","a":"65000.1","A":"2"}"#;
        let CoinmDecoded::Quote {
            update_id,
            bid_price,
            ask_qty,
            ..
        } = decode_text(quote).unwrap()
        else {
            panic!("quote");
        };
        assert_eq!(update_id, 9);
        assert_eq!(bid_price.0, Fixed::new(650000, 1));
        assert_eq!(ask_qty.0, Fixed::new(2, 0));

        let liq = br#"{"e":"forceOrder","E":5,"o":{"s":"BTCUSD_PERP","S":"SELL","o":"LIMIT","f":"IOC","q":"1","p":"9900","ap":"9910","X":"FILLED","l":"1","z":"1","T":5}}"#;
        let CoinmDecoded::ForceOrder { side, price, .. } = decode_text(liq).unwrap() else {
            panic!("forceOrder");
        };
        assert_eq!(side, AggressorSide::Sell);
        assert_eq!(price.0, Fixed::new(9910, 0));

        let oi = br#"{"symbol":"BTCUSD_PERP","openInterest":"10659.509","time":1589437530011}"#;
        let CoinmDecoded::OpenInterest { quantity, .. } = decode_text(oi).unwrap() else {
            panic!("oi");
        };
