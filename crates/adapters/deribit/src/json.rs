//! Shared JSON deserialize helpers for Deribit JSON-RPC hot-path decode.
//!
//! # ponytail
...
mod tests {
    ...
    fn serde_backend_roundtrip() {
        let raw = br#"{"method":"subscription"}"#;
    ...
    fn serde_simd_value_and_typed_parity() {
        let raw = br#"{"method":"subscription","nested":{"x":1}}"#;
        ...
        assert_eq!(ta, tb);
        assert_eq!(ta.method, "subscription");
    }
}
