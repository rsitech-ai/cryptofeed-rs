//! Shared JSON deserialize helpers for Bybit V5 hot-path decode.
//!
//! # ponytail
...
mod tests {
    ...
    fn serde_backend_roundtrip() {
        let raw = br#"{"topic":"publicTrade.BTCUSDT"}"#;
        let v: Tiny = from_slice_serde(raw).unwrap();
        assert_eq!(
            v,
            Tiny {
                topic: "publicTrade.BTCUSDT".into(),
            }
        );
    }
    ...
    fn serde_simd_value_and_typed_parity() {
        let raw = br#"{"topic":"publicTrade.BTCUSDT","nested":{"x":1}}"#;
