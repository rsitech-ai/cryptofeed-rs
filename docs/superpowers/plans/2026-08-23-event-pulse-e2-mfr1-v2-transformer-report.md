...
- Archive repair REDs: the first regression API did not exist (`E0425`), proving the repository-only attribute check had not been separated from unconditional fixture verification; the successor regression also failed with `E0425` because no unavailable-Git command path existed, exposing the caller's repository-presence assertion.
- GREEN: the review-hardened focused suite passes 20/20. In addition to the initial matrix, it rejects wrong-route, legacy, advanced, and wrong-config machines before replay; binds complete metadata; proves E/T causal separation; exercises exact 65,536/65,537 raw-record boundaries; proves DropNewest and FailEngine behavior; and proves no partial return after a late replay failure. The canonical oracle's 7 lines, 7,736 bytes, absence of CR bytes, and SHA-256 `a65c1f39f7dc0150748d0f0facb0ea6cc09ca0dcedeaaff07284513c90040237` are always checked. Only the LF Git-attribute assertion is skipped when Git cannot be spawned or `git rev-parse --show-toplevel` fails; successful repository discovery requires strict attributes. Temporary non-repository and missing-Git regressions prove both archive paths.
