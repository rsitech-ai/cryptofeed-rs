...
- 2026-08-11: Real-browser live verification covered chart modes, assets, order-flow, DOM, Market Profile, markets, settings, replay gating, and narrow/desktop layout. The browser reported zero warnings/errors, 248 API resources, zero failed resources, and active SSE.
- 2026-08-11: A failed long attempt exposed unexplained Binance USD-M reconnects. Added structured reconnect cause/backoff logging, then reproduced a raw WebSocket code `1006` alongside the daemon's `Connection reset without closing handshake`; the daemon recovered 5/5 books without invalidations or drops.
- 2026-08-11: Exact clean 15-minute run `20260811T184959Z` passed with 13/13, zero reconnects, 119.86 MiB peak RSS, zero measured RSS growth, clean logs, first-pass smoke, and graceful shutdown.
- 2026-08-11: Exact clean one-hour run `20260811T190518Z` passed with 13/13 minimum, per-venue reconnect allowances respected, 172.06 MiB peak RSS, 40.10 MiB/hour fitted growth, 36.73 MiB/hour p95-envelope growth, API p95 1.330 ms, CPU p95 7.0%, first-pass smoke, and graceful shutdown.
- 2026-08-11: Milestones M1-M4 completed. M5 proceeds through final repository-template checks, exact diff review, PR creation/readback, and merge verification; any failure restores HOLD.
